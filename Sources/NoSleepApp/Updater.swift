...
final class Updater: ObservableObject {
    ...
    /// Il click cambia stato subito; la rete prosegue senza fermare il pannello.
    func checkNow() {
    ...
    func perform(_ action: Updates.Action) {
        ...
        case .upgradeAndRelaunch(let arguments):
            guard let brew = brewURL() else {
                state = .failed(S.updatesBrewMissing)
                printBenchState()
                if let releasePage { open(releasePage) }
                return
            }
            state = .upgrading(line: S.updatesPreparing)
            printBenchState()
    ...
    private func check() async {
        defer { defaults.set(Date(), forKey: "updates.lastCheck") }
        ...
        do {
            ...
            let roots = environment["NOSLEEP_CASKROOM"].map { [$0] }
            ...
            let bundlePath = benchMode ? "/Applications/NoSleep.app" : Bundle.main.bundlePath
            ...
            model.record(S.logUpdateCheckFailed(error.localizedDescription))
            state = .idle
            printBenchState()
        }
    }
    ...
    private func releaseData() async throws -> Data {
        let rawURL = environment["NOSLEEP_UPDATES_API"]
    ...
    private func brewURL() -> URL? {
        ...
        if let selected = environment["NOSLEEP_BREW"] {
            candidates = [selected]
        } else {
    ...
    private func upgrade(with brew: URL, arguments: [String]) async {
        ...
        guard result.status == 0 else {
            ...
            state = .failed(reason.isEmpty ? S.updatesProcessFailed : reason)
            printBenchState()
            return
        }
        ...
        relaunchAfterExit()
        model.releaseEverythingAndQuit()
    }

    /// Homebrew 6 lascia `com.apple.quarantine` sull'app che ha appena installato (misurato il
    /// 30/08/2026 sulla 6.0.20: `--no-quarantine` non esiste più). Senza questa riga ogni
    /// aggiornamento riaprirebbe l'avviso di macOS sul non notarizzato. Si tocca SOLO il proprio
    /// bundle, che è lo stesso gesto di `install.sh`; un fallimento non blocca il riavvio.
    private func clearQuarantine() async {
        ...
        if result.status != 0 {
            model.record(S.logUpdateQuarantineKept(result.lines.last ?? ""))
        }
    }
...
private enum UpdateError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .invalidURL: return S.updatesInvalidURL
        case .badResponse: return S.updatesBadResponse
        case .releaseWithoutTag: return S.updatesReleaseWithoutTag
        }
    }
}
...
private enum ProcessRunner {
    ...
    static func run(executable: URL,
        ...
        await withCheckedContinuation { continuation in
            ...
            do {
                try process.run()
                // La copia di scrittura del genitore deve chiudersi: altrimenti, finito il figlio,
                // la lettura aspetta un EOF che il genitore stesso tiene ancora aperto.
